...
The framework defines four strict memory types:

### 2.1 short_term_context
*Current task, workflow, and inputs*
- Recent messages
- Active risks and dependencies

### 2.2 mid_term_context
*Current sprint and roadmap cycle*
- Active decisions, ongoing risks, and pending escalations

### 2.3 long_term_context
*System models, workflows, templates, and personas*
- Organizational rules and metrics definitions

### 2.4 ephemeral_memory
*Temporary reasoning artifacts and calculations*
- Ephemeral memory must be discarded immediately after output generation.
...
AI agents must retrieve context in the following explicit order. During retrieval, all `short_term_context` and `mid_term_context` must be cross-referenced against `ai-ops-system-knowledge-model.md` to ensure operational consistency:
...
Agents must not retrieve context outside the scope of the active task.
...
This ensures structured reasoning and deterministic outputs.
...
- Prioritize the most recent context based on metadata timestamps.
- Avoid merging unrelated contexts.

## 7. Forgetting & Archiving Rules

AI agents must:
- Discard `ephemeral_memory` after output generation.
...
- Clear persona context when a persona is unloaded.
...
- Follow redaction, auditability, and security classification rules defined in `ai-ops-system-security-compliance.md`.
...
- Storing unnecessary data or executing incorrect context stitching.
- Missing risk or alignment memory.

These failures instantly trigger diagnostic evaluations mapped in `ai-ops-system-drift-detection.md`.
